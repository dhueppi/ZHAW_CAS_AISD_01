...
\begin{longtable}{@{}p{8.2cm}@{\hspace{2cm}}p{12.6cm}@{}}
...
{Neugier, Vorfreude -- ÖV als selbstverständlicher Ermöglicher.}
...
{Gelassen, souverän -- kein Stress, Entscheidung fällt leicht.}
...
{Entspannt wenn alleine; leicht frustriert über das Auto-Umfeld.}
...
{Fokussiert, neugierig -- Planung macht Spass.}
...
{Freude am Optimieren; leichte Irritation bei unklaren Regeln.}
...
{Routiniert, entspannt -- nur bei Anschlussbillett-Fragen kurz unsicher.}
...
{Positiv, aktiv -- Bewegung als Genuss.}
...
{Zufrieden, erholt; leicht genervt bei Überfüllung.}
...
{Kompetent, sicher; Stress nur bei unerwarteten Ausfällen.}
...
{Entspannt bis leicht genervt (Gedränge); müde, aber zufrieden.}
...
{Dankbar, positiv bestätigt -- ÖV-Affinität wird bekräftigt.}
...
\clearpage
\subsubsection{How-Might-We-Gesamtliste (konsolidiert)}
\label{anh:hmw}
\textit{Quelle: \texttt{define/HMW\_Kurt\_Gesamtliste.docx} -- konsolidierte Gesamtliste, zusammengeführt aus \texttt{define/Archive/HmW Fragen.xlsx}, \texttt{define/Archive/HMW\_KurtKreuzUndQuer\_ZVV.docx} und \texttt{define/Archive/How Might We Fragen.docx}. Leitfrage: Mit welchen Massnahmen können wir vitale, bislang überwiegend autoabhängige Senior:innen im Kanton Zürich zur verstärkten ÖV-Nutzung für Freizeitmobilität motivieren?}

\paragraph{Botschafter \& Multiplikator}
\begin{itemize}[nosep]
  \item Wie könnten wir versierte ÖV-Nutzer motivieren, aktiv als Botschafter in ihrem Umfeld aufzutreten?
  \item Wie könnten wir Kurt eine offizielle, vom ZVV anerkannte und sichtbare ÖV-Botschafter-Rolle geben?
  \item Wie könnten wir eine Gemeinschaft von ÖV-Botschaftern aufbauen, die sich gegenseitig mit Ausflugideen und Tipps unterstützen?
  \item Wie könnten wir Kurt eine ÖV-Identität geben, die so stark ist, dass andere fragen: \enquote{Wie bist du dahin gekommen?}
\end{itemize}

\paragraph{Wissen weitergeben}
\begin{itemize}[nosep]
  \item Wie könnten wir Kurts Erfahrungswissen nutzbar und sichtbar machen, ohne dass er Extraaufwand hat und ohne dass Mitreisende sich belehrt fühlen?
  \item Wie könnten wir verhindern, dass Kurts Wissen mit der Pensionierung verschwindet -- und es stattdessen zum Start seiner grössten Wirkung machen?
\end{itemize}

\paragraph{Planen \& Organisieren}
\begin{itemize}[nosep]
  \item Wie könnten wir versierten ÖV-Nutzern das Planen und Organisieren eines Gruppenausflugs leichter machen?
  \item Wie könnten wir Tickets und Verbindungen für eine ganze Gruppe auf einmal lösen?
  \item Wie könnten wir ÖV-Ausflüge so organisieren, wie es ein Verkehrsverein mit festen Sonntags-Gruppenfahrten täte?
\end{itemize}

\paragraph{Mitreisende einbinden \& erster Eindruck}
\begin{itemize}[nosep]
  \item Wie könnten wir Nicht-ÖV-Nutzer so einbinden, dass sie sich um nichts kümmern müssen?
  \item Wie könnten wir den ersten gemeinsamen Ausflug so gestalten, dass er einen bleibenden positiven Eindruck hinterlässt?
  \item Wie könnten wir den Moment des Ankommens -- entspannt, ohne Parkplatzsuche -- erlebbar und erinnerungswürdig machen?
  \item Wie könnten wir den Ausflug so führen, wie ein Wanderleiter eine Gruppe sicher und mit Vorfreude durch unbekanntes Gelände begleitet?
\end{itemize}

\paragraph{Vom Mitfahren zum Selbstfahren}
\begin{itemize}[nosep]
  \item Wie könnten wir Nicht-Nutzern nach einem gemeinsamen Ausflug den Einstieg in die eigenständige ÖV-Nutzung erleichtern?
  \item Wie könnten wir das Gefühl von Souveränität vermitteln, wenn jemand das erste Mal allein in den Zug steigt?
  \item Wie könnten wir den Moment des ersten eigenständigen Tickets zum Kinderspiel machen?
\end{itemize}

\paragraph{(Soziales) Erlebnis}
\begin{itemize}[nosep]
  \item Wie könnten wir den sozialen Aspekt stärken, sodass die Fahrt als Erlebnis und nicht nur als Transport wahrgenommen wird?
  \item Wie könnten wir Zugehörigkeit vermitteln, wenn Kurt mit Vereinskolleg:innen gemeinsam plant und einsteigt?
  \item Wie könnten wir die soziale ÖV-Erfahrung auch für ein neu zugezogenes Vereinsmitglied gestalten, das niemanden kennt?
  \item Wie könnten wir den ÖV-Ausflug so \enquote{unvergesslich}, \enquote{überraschend} und \enquote{spielerisch} gestalten, dass das Erlebnis der Fahrt selbst -- nicht nur das Ziel -- zum Gesprächsthema unter Freunden wird?
  \item Wie könnten wir Kurts Erfahrungswissen über Routen, Anschlüsse und schöne Ausflugsziele für andere Senior:innen nutzbar machen?
\end{itemize}

\paragraph{Motivation \& Gewohnheit}
\begin{itemize}[nosep]
  \item Wie könnten wir Kurts Freude am Mitnehmen und Routenplanen so verstärken, dass eine feste soziale Gewohnheit entsteht?
  \item Wie könnten wir die anfängliche Scheu vor dem ÖV zum Startpunkt eines gemeinsamen Erfolgserlebnisses machen?
  \item Wie könnten wir autoabhängige Senior:innen gerade dadurch bewegen, dass sie selbst jemanden mitnehmen?
  \item Wie könnten wir eine Freizeitwelt schaffen, in der das Auto für vitale Senior:innen nicht mehr der selbstverständliche Standard für Ausflüge ist?
\end{itemize}

\paragraph{Tracking \& Messen}
\begin{itemize}[nosep]
  \item Wie könnten wir Kurts Freude am Messen (Kilometer, Orte) mit seinen ÖV-Reisen verbinden, sodass jede Fahrt zum messbaren Erlebnis wird?
  \item Wie könnten wir aus vielen ÖV-Reisen eine persönliche Mobilitätsbiografie aufbauen?
  \item Wie könnten wir die Tracking-Motivation auch ohne Smartphone aktivieren?
\end{itemize}

\paragraph{Tarif \& Zugänglichkeit}
\begin{itemize}[nosep]
  \item Wie könnten wir Senior:innen für den ÖV begeistern, ohne anzunehmen, dass sie zuerst App oder Smartphone beherrschen müssen?
  \item Wie könnten wir Tarif- und Zonenkomplexität für die Reisenden unsichtbar machen?
  \item Wie könnten wir Kurts Frustration über unlogische Regeln direkt in Systemverbesserungen übersetzen?
  \item Wie könnten wir die ÖV-Freizeitnutzung mit Menschen wie Kurt fördern, statt mit Technikbudget?
  \item Wie könnten wir Kurts ÖV-Alltag so kuratieren, wie Spotify eine Discover Weekly zusammenstellt -- vertraut, aber mit überraschenden Verbindungen?
\end{itemize}

\paragraph{Ergänzungen aus der ZVV-Fallstudie (Reframing-Strategien)}
\begin{itemize}[nosep]
  \item \emph{Question the assumption:} Wie könnten wir einen Gruppenausflug gestalten, der die Annahme widerlegt, dass der ÖV für Freizeitausflüge in der Gruppe weniger bequem und flexibel ist als Auto oder Car?
  \item \emph{Go after adjectives:} Wie könnten wir die Fahrt selbst so unvergesslich, überraschend und spielerisch machen, dass sie -- nicht nur das Ziel -- zum Gesprächsthema unter Freunden wird?
  \item \emph{Identify unexpected resources:} Wie könnten wir Kurts Vereinsnetzwerk und lokales Wissen nutzen, um ÖV-Ausflüge für Gleichgesinnte zu kuratieren, ohne dass der ZVV alle Inhalte selbst produzieren muss?
  \item \emph{Create an analogy:} Wie könnten wir dem ÖV-Ausflug das Erlebnis eines geführten Kulturspaziergangs geben -- wie ein Audioguide auf Schienen, der Wissen über Landschaft, Orte und Geschichte direkt auf der Fahrt erlebbar macht?
  \item \emph{Play POV against the challenge:} Wie könnten wir Kurts Argument \enquote{der ÖV fährt überall hin} auch für überzeugte Autofahrer in seiner Gruppe erlebbar und unwidersprechlich machen?
  \item \emph{Explore the opposite:} Wie könnten wir den Ausflug selbst so unwiderstehlich gestalten, dass die Mitreisenden von sich aus fragen: \enquote{Wie kommen wir da am besten hin?}
  \item \emph{Break POV into pieces:} Wie könnten wir den Einladungsmoment -- wenn Kurt jemanden zum ersten Mal zu einem ÖV-Ausflug einlädt -- so einfach und einladend gestalten, dass die Antwort fast immer \enquote{Ja} ist?
\end{itemize}
